...
let selected=null,world=null,scale=.4,calls=0,logs=[];
...
function select(id,from){
  ...
  $("properties").textContent=JSON.stringify(attributes(node),null,2);
}
...
function renderCanvas(){
  shapes=new Map();const svg=$("canvas");svg.replaceChildren();world=extent(nodes);
...
function renderTree(){
  ...
  function branch(id){
    ...
    const row=document.createElement(n.children.length?"summary":"div");row.className="node-row";row.dataset.id=id;
...
async function load(){
  ...
  nodes=new Map();selected=null;world=null;calls=0;logs=[];document.body.dataset.selected="";
  ...
  try{
    const response=await request("/api/session",{case:$("cases").value},signal);active();
    ...
    nodes=result;renderTree();renderCanvas();fit();$("empty").hidden=true;
